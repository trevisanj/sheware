...
#    self.data = t_experiment.get_cursor(field_names=("experiment.id", "experiment.name", "count(*) as no_spectra"), 
#    joins=[('spectrum', 'spectrum.idexperiment = experiment.id')], groupbys=['experiment.id']).fetchall()
    self._data = t_experiment.get_cursor(field_names=("experiment.id", "experiment.name"), orderbys="experiment.name")

    self.data = []
    for (id, name) in self._data:
      count = t_spectrum.get_cursor_data(field_names=["count(*)"], wheres=("idexperiment = %s" % id,)).fetchall()[0][0]
      self.data.append((id, name, count))
  ...
  def populate_choice_domain(self):
    ...
    idexperiment = self.get_idexperiment()
    if idexperiment > 0:
      self.data_domain = t_domain.get_cursor(field_names=("id", "name", "wncount")).fetchall()

      for (iddomain, name, wncount) in self.data_domain:
        no_series = t_series.get_count_1experiment(idexperiment, iddomain)
        self.choiceDomain.Append("%s (%s features) (%d series)" % (name, wncount, no_series))
    else:
      self.data_domain = ()
